...
REM ############################################################################
REM # UNIX-style sha512sum.                                                    #
REM ############################################################################

REM Ignore directories.
IF EXIST %1\* (
    ECHO sha512sum: %1: Is a directory
    EXIT /B
)
...
FOR %%F IN ("%1") DO (
    ...
    REM Process hash.
    SET COUNT=1
    FOR /F "tokens=* USEBACKQ" %%L IN (`POWERSHELL -NOPROFILE -EXECUTIONPOLICY BYPASS -COMMAND "$(CertUtil -hashfile \""!FILE!\"" SHA512)[1] -replace \"" \"",\""\"""`) DO (
        SET LINE!COUNT!=%%L
        SET /A COUNT=!COUNT!+1
    )
    ...
    REM File does not exist; show error.
        ...
        ECHO sha512sum: !FILE!: No such file or directory
    )
)
ENDLOCAL
